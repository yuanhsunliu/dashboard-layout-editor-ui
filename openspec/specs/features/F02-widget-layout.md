# F02: Widget Layout

**Status**: ✅ Implemented (2026-01-05)

## Overview
使用者可以在 Dashboard 編輯頁中新增 Widget，並透過拖放調整 Widget 的位置與大小。

## User Stories
- 作為使用者，我可以在 Dashboard 中新增 Widget，以便放置圖表
- 作為使用者，我可以拖放 Widget 調整位置，以便自訂佈局
- 作為使用者，我可以調整 Widget 大小，以便控制圖表顯示區域
- 作為使用者，我可以刪除不需要的 Widget

## Acceptance Criteria

### 新增 Widget
- [x] 點擊「新增 Widget」按鈕可新增空白 Widget
- [x] 新 Widget 自動放置在可用空間
- [x] 新 Widget 有預設大小 (w: 4, h: 3)

### 拖放移動
- [x] 可拖曳 Widget 標題列移動位置
- [x] 移動時顯示放置預覽
- [x] 放開後 Widget 固定在新位置
- [x] Widget 之間自動避免重疊

### 調整大小
- [x] Widget 右下角有 resize handle
- [x] 可拖曳調整寬度與高度
- [x] 有最小尺寸限制 (minW: 2, minH: 2)

### 刪除 Widget
- [x] Widget 右上角有刪除按鈕
- [x] 點擊後直接刪除（無確認 Dialog）
- [x] 刪除後其他 Widget 不自動重排

### 佈局儲存
- [x] 佈局變更自動同步到狀態
- [x] 自動儲存 (500ms debounce)

## UI/UX Spec

### Dashboard 編輯頁 (`/dashboard/:id`)
```
┌─────────────────────────────────────────────────┐
│  ← 返回    Dashboard 名稱 ✏️     [+ 新增 Widget] │
├─────────────────────────────────────────────────┤
│                                                 │
│  ┌─────────────────┐  ┌───────────────────────┐ │
│  │ Widget 1     [x]│  │ Widget 2           [x]│ │
│  │                 │  │                       │ │
│  │   (圖表區域)    │  │      (圖表區域)       │ │
│  │                 │  │                       │ │
│  │              ◢  │  │                    ◢  │ │
│  └─────────────────┘  └───────────────────────┘ │
│                                                 │
│  ┌───────────────────────────────────────────┐  │
│  │ Widget 3                               [x]│  │
│  │                                           │  │
│  │              (圖表區域)                   │  │
│  │                                        ◢  │  │
│  └───────────────────────────────────────────┘  │
│                                                 │
└─────────────────────────────────────────────────┘
```

### Widget 元件
- 標題列：可拖曳移動
- 內容區：顯示圖表（或空白提示）
- 刪除按鈕：右上角 X icon
- Resize handle：右下角

### 空白 Widget
```
┌─────────────────┐
│ 新增 Widget  [x]│
│                 │
│   📊 點擊設定   │
│   選擇圖表類型  │
│              ◢  │
└─────────────────┘
```

## Data Model

```typescript
// react-grid-layout 格式
interface LayoutItem {
  i: string;      // Widget ID
  x: number;      // Grid X 位置
  y: number;      // Grid Y 位置
  w: number;      // 寬度 (grid units)
  h: number;      // 高度 (grid units)
  minW?: number;  // 最小寬度
  minH?: number;  // 最小高度
}

interface Widget {
  id: string;
  chartConfig?: ChartConfig;  // 尚未設定時為 undefined
}
```

### Grid 設定
```typescript
const GRID_CONFIG = {
  cols: 12,           // 12 欄 grid
  rowHeight: 80,      // 每行高度 80px
  margin: [16, 16],   // 間距 16px
};

const DEFAULT_WIDGET_SIZE = {
  w: 4,
  h: 3,
  minW: 2,
  minH: 2,
};
```

## State Management

```typescript
// stores/useDashboardStore.ts
interface DashboardState {
  layout: LayoutItem[];
  widgets: Widget[];
  
  // Actions
  addWidget: () => void;
  removeWidget: (id: string) => void;
  updateLayout: (layout: LayoutItem[]) => void;
}
```

## Dependencies
- T01: Project Setup
- F01: Dashboard CRUD（需要 Dashboard 編輯頁）

## Out of Scope
- Widget 複製
- Widget 鎖定位置
- 佈局模板
- 響應式佈局（不同螢幕尺寸）
