...
interface FieldMappingFormProps {
  ...
  onYAxisChange: (values: string[]) => void;
  errors?: {
    xAxisField?: string;
    yAxisFields?: string;
  };
...
export function FieldMappingForm({
  ...
  const handleYAxisToggle = (fieldName: string, checked: boolean) => {
    if (checked) {
      onYAxisChange([...yAxisFields, fieldName]);
    } else {
      onYAxisChange(yAxisFields.filter(f => f !== fieldName));
    }
  };
  ...
  return (
    <div className="space-y-4" data-testid="field-mapping-form">
      <div className="space-y-2">
        <Label>X 軸欄位</Label>
        <Select value={xAxisField} onValueChange={onXAxisChange}>
            ...
            numberFields.map((field) => (
              <div key={field.name} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={yAxisFields.includes(field.name)}
